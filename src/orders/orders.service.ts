    ...
    async createOrder(payload: any) {
        ...
        const order = await this.prisma.order.create({
            data: {
                ...
                status: 'pending',
                items: {
                    create: items.map((i: { menuItemId: any; quantity: any; price: any; notes: any; }) => ({ menuItemId: i.menuItemId, quantity: i.quantity, price: i.price || 0, notes: i.notes }))
                }
            },
            include: { items: true }
        });
        ...
        // calculate total
        const total = order.items.reduce((s: number, it: { price: number; quantity: number; }) => s + it.price * it.quantity, 0);
